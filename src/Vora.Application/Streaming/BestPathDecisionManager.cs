...
public class BestPathDecisionManager : IBestPathDecisionManager
{
    ...
    public async Task<StreamDecisionDto> DetermineBestPathAsync(ClientDevice client, MediaStreamInfoDto mediaItem, int maxAllowedBandwidthKbps, string bandwidthLimitSource, Guid? requestedVideoId = null, Guid? requestedAudioId = null, Guid? requestedSubtitleId = null, int requestedMaxResolution = 0, Guid? requestedMediaPartId = null)
    {
        ...
        foreach (var part in partsToEvaluate)
        {
            ...
            // What the client can decode and display, versus what the encoder
            // can actually hand it. A 1344p panel is served 1080p, so scoring
            // has to compare against 1080 — crediting an option with delivering
            // 1344p made a 4K transcode look like a perfect fit while a 1080p
            // part that direct-plays was penalised for missing a height no file
            // and no encoder can produce. clientMaxRes stays raw for the
            // capability test below: that one is about the device's limit, and
            // snapping it would let a 1440p source through to this panel.
            int deliverableMaxRes = SnapToDeliverableHeight(clientMaxRes);
            ...
            int effectiveTargetHeight = deliverableMaxRes;
            ...
            {
                subTrack = PickAutomaticSubtitle(part.SubtitleTracks, primaryAudioTrack?.Language, settings);
            }
            ...
            foreach (var audioTrack in audioTracksToEvaluate)
            {
                ...
                //     user/client asked for. deliverableMaxRes is the
                //     ceiling snapped to a rung the encoder can emit
                //     (RequestedMaxResolution or 2160 default);
                //     outputHeight is what this part can actually
                //     deliver under the current HDR-downscale rules.
                //     Both sides must be ladder heights: scored against
                //     a raw 1344 no option can ever reach zero, and a
                //     4K downscale to 1344 scored as a perfect fit
                //     while the 1080p part that direct-plays did not.
                ...
                int qualityLossPenalty = Math.Max(0, deliverableMaxRes - outputHeight) * 10;
                ...
                decisionLogs.Add(new StreamOptionLogDto
                {
                    ...
                    ClientMaxHeight = clientMaxRes,
                    DeliverableHeight = deliverableMaxRes,
    ...
    // The heights the transcoder can actually emit. A client ceiling that falls
    // between rungs (a 1344p phone panel, a 1600p tablet) can only be served at
    // the rung below it, so that is what the scorer must compare against.
    //
    // The rungs are deliberately identical to FormatHeightAsResolution: if the
    // two ever diverge, the resolution the decision declares and the one it
    // scored disagree, which is the whole of the bug this exists to stop.
    private static int SnapToDeliverableHeight(int height)
    {
    ...
    // What plays when the client asked for a part but not for a subtitle. The
    // flag-driven pick is the baseline; AutoEnableSubtitlesForForeignAudio adds
    // the case a flag cannot express — a title whose own audio is in a language
    // this server does not read, where the right track is the one in the
    // server's language even though nothing in the file marks it as special.
    //
    // An audio track with no language tag is left to the baseline. Most of those
    // are ordinary local rips in the server's own language, and reading "no tag"
    // as "foreign" would put subtitles on half a library.
    public static SubtitleStreamInfoDto? PickAutomaticSubtitle(List<SubtitleStreamInfoDto> tracks, string? audioLanguage, ServerSetting settings)
    {
        var flagged = tracks.FirstOrDefault(s => s.IsForced) ?? tracks.FirstOrDefault(s => s.IsDefault);

        if (!settings.AutoEnableSubtitlesForForeignAudio) return flagged;
        if (MetadataLanguageCodes.Normalize(audioLanguage) == null) return flagged;
        if (MetadataLanguageCodes.SameLanguage(audioLanguage, settings.MetadataLanguage)) return flagged;

        var inServerLanguage = tracks
            .Where(s => MetadataLanguageCodes.SameLanguage(s.Language, settings.MetadataLanguage))
            .ToList();

        if (inServerLanguage.Count == 0) return flagged;

        return inServerLanguage.FirstOrDefault(s => s.IsForced && !s.IsHearingImpaired)
            ?? inServerLanguage.FirstOrDefault(s => s.IsForced)
            ?? inServerLanguage.FirstOrDefault(s => !s.IsHearingImpaired)
            ?? inServerLanguage[0];
    }
