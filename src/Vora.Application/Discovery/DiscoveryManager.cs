...
{
    // Every row on the Discover page comes from one of these providers, and one
    // without a key returns none at all. No provider configured is therefore an
    // empty page, which is what the nav must not offer.
    public async Task<bool> IsAvailableAsync(CancellationToken cancellationToken = default)
    {
        foreach (var plugin in plugins)
        {
            if (await plugin.IsConfiguredAsync(cancellationToken)) return true;
        }
