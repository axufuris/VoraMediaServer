...
public class DvrPostProcessingWorker : BackgroundService
{
    ...
    private async Task ProcessCompletedRecordingsAsync()
    {
        ...
        var notifier = scope.ServiceProvider.GetRequiredService<Vora.Application.Analysis.IClientNotifier>();
    ...
    private async Task ProcessSingleRecordingAsync(IptvRecordingSession session, IIptvRepository repo, Vora.Application.Settings.ISystemSettingsRepository settingsRepo, Vora.Application.Analysis.IClientNotifier notifier)
    {
    ...
    private async Task<string> BuildFfmpegArgsAsync(string inputPath, string outputPath, Vora.Application.Settings.ISystemSettingsRepository settingsRepo)
    {
    ...
    private async Task<string> RunComskipAndGetMarkersAsync(IptvRecordingSession session, string videoPath, IIptvRepository repo, Vora.Application.Analysis.IClientNotifier notifier)
    {
        ...
        try
        {
            ...
            {
                FileName = "comskip",
                Arguments = $"--quiet --ini=\"{iniPath}\" \"{videoPath}\"",
                CreateNoWindow = true,
                UseShellExecute = false,
            ...
            if (comskipProc != null) await comskipProc.WaitForExitAsync();
