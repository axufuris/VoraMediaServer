...
public class OpenAiRecommendationProvider : IRecommendationProvider
{
    ...
    public IEnumerable<PluginSettingDefinitionDto> GetSettingDefinitions()
    {
        return new List<PluginSettingDefinitionDto>
        {
            ...
            new PluginSettingDefinitionDto { Key = "collections_model", Label = "AI Collections Model", Type = "select", DefaultValue = "gpt-4.1", Options = ChatModels, Description = "Model used for AI collection lists and chronological ordering, which benefit from stronger reasoning and more up-to-date knowledge. gpt-4.1 is recommended; the -mini models are cheaper but weaker at ordering." },
