...
public interface ISubtitleSearchManager
{
    ...
    Task<DownloadedSubtitleVM?> DownloadAndAttachAsync(Guid mediaItemId, string providerFileId, string? language, bool hearingImpaired = false, bool forced = false, CancellationToken cancellationToken = default);
}
...
public class SubtitleSearchManager : ISubtitleSearchManager
{
    ...
    public async Task<DownloadedSubtitleVM?> DownloadAndAttachAsync(Guid mediaItemId, string providerFileId, string? language, bool hearingImpaired = false, bool forced = false, CancellationToken cancellationToken = default)
    {
        var provider = await ResolveProviderAsync(cancellationToken);
        ...
        var track = new MediaSubtitleTrack
        {
            ...
            Title = BuildTitle(resolvedLanguage, provider.Name, hearingImpaired),
            IsForced = forced,
    ...
    public static string BuildTitle(string? language, string providerName, bool hearingImpaired = false)
    {
        var lead = string.IsNullOrWhiteSpace(language)
            ? (hearingImpaired ? "SDH" : null)
            : (hearingImpaired ? $"{language.ToUpperInvariant()} SDH" : language.ToUpperInvariant());

        return lead == null ? providerName : $"{lead} ({providerName})";
    }
