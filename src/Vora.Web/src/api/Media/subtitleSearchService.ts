...
export const subtitleSearchService = {
    ...
    // The provider's download response is just bytes, so whether the file is SDH
    // or forced is only knowable from the search row the viewer picked — it goes
    // back up with the request or the attached track loses those labels.
    download: async (mediaId: string, result: SubtitleSearchResult, serverId?: string): Promise<DownloadedSubtitle> => {
