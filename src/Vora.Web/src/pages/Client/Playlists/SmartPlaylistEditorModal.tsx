...
export default function SmartPlaylistEditorModal({ serverId, initialId, initialMediaType = 'Music', onClose, onSaved }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-[var(--vora-bg-raised)] border border-[var(--vora-border-subtle)] rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-[var(--vora-bg-raised)] border-b border-[var(--vora-border-subtle)] px-6 py-4 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-[var(--vora-text-primary)] flex items-center gap-2">
                    ...
                    <button onClick={onClose} className="text-[var(--vora-text-muted)] hover:text-[var(--vora-text-primary)] cursor-pointer text-2xl leading-none">×</button>
                </div>

                <div className="p-6 space-y-6">
                    {loading ? (
                        <div className="text-[var(--vora-text-muted)]">Loading…</div>
                    ) : (
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-[var(--vora-bg-canvas)] border border-[var(--vora-border-subtle)] rounded-md p-2 text-[var(--vora-text-primary)] outline-none focus:border-fuchsia-500"
                            ...
                            <div>
                                <div className="text-sm font-bold text-[var(--vora-text-secondary)] mb-2">Rules</div>
                                <RuleGroupEditor
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-[var(--vora-border-subtle)]">
                                <div>
                                    <label className="block text-xs font-bold text-[var(--vora-text-muted)] uppercase tracking-wide mb-2">Sort by</label>
                                    <select
                                        value={definition.sortBy}
                                        onChange={e => setDefinition(d => ({ ...d, sortBy: e.target.value as SmartPlaylistSortBy }))}
                                        className="w-full bg-[var(--vora-bg-canvas)] border border-[var(--vora-border-subtle)] rounded p-2 text-[var(--vora-text-primary)] outline-none cursor-pointer"
                                    >
                                        {sortOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-[var(--vora-text-muted)] uppercase tracking-wide mb-2">Direction</label>
                                    <select
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-[var(--vora-text-muted)] uppercase tracking-wide mb-2">Limit</label>
                                    <input
                                        type="number"
                                        min={0}
                                        max={5000}
                                        ...
                                        className="w-full bg-[var(--vora-bg-canvas)] border border-[var(--vora-border-subtle)] rounded p-2 text-[var(--vora-text-primary)] outline-none"
                                        placeholder="(no limit)"
                                    />
                                </div>
                            </div>

                            <div className="flex items-center gap-3 text-sm text-[var(--vora-text-muted)] pt-2 border-t border-[var(--vora-border-subtle)]">
                                <span>Preview:</span>
                                <span className="font-bold text-[var(--vora-text-primary)]">
                ...
                <div className="sticky bottom-0 bg-[var(--vora-bg-raised)] border-t border-[var(--vora-border-subtle)] px-6 py-4 flex justify-end gap-3">
                    <button onClick={onClose} disabled={saving} className="px-4 py-2 rounded text-[var(--vora-text-secondary)] hover:bg-[var(--vora-bg-sunken)] transition-colors cursor-pointer">Cancel</button>
                    <button onClick={handleSave} disabled={saving} className="px-6 py-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-[var(--vora-text-primary)] font-bold rounded shadow-lg transition-colors cursor-pointer disabled:opacity-50">
    ...
    return (
        <div className={`${indent} space-y-2`}>
            ...
            {group.groups.map((sub, idx) => (
                <div key={`g-${idx}`} className="relative">
                    <button
                        ...
                        className="absolute -left-2 top-0 w-5 h-5 rounded-full bg-rose-700 hover:bg-rose-500 text-[var(--vora-text-primary)] text-xs cursor-pointer z-10"
                        title="Remove group"
                    >×</button>
                    <RuleGroupEditor
                        group={sub}
            ...
            <div className="flex gap-2 pt-1">
                <button onClick={() => onAddRule(path)} className="px-3 py-1 text-xs font-bold bg-[var(--vora-bg-sunken)] hover:bg-[var(--vora-bg-sunken)] text-[var(--vora-text-primary)] rounded transition-colors cursor-pointer">+ Rule</button>
                <button onClick={() => onAddGroup(path)} className="px-3 py-1 text-xs font-bold bg-[var(--vora-bg-sunken)] hover:bg-[var(--vora-bg-sunken)] text-[var(--vora-text-primary)] rounded transition-colors cursor-pointer">+ Group</button>
            </div>
        </div>
    );
}
