...
export default function DiscoveryCustomizeModal({ isOpen, onClose, configs, savedLayout, onSave }: Props) {
    ...
    return (
        ...
        >
            <div className="mb-6 border-b border-gray-800 pb-4 shrink-0">
                <h2 className="text-2xl font-bold text-white mb-2">Customize Discovery</h2>
                <p className="text-gray-400 text-sm">Drag to reorder rows. Toggle to hide lists you don't want to see. Saved to this device.</p>
            </div>
            ...
            <ul className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-2">
                {layoutItems.map((item, index) => (
                    <li
                        ...
                        className={`flex items-center justify-between p-4 bg-gray-800 border ${draggedIndex === index ? 'border-orange-500 opacity-50' : 'border-gray-700'} rounded-lg cursor-grab hover:bg-gray-700 transition-colors shadow-sm`}
                    >
                        <div className="flex items-center gap-4">
                            <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8h16M4 16h16" /></svg>
                            <div>
                                <p className={`font-bold text-sm ${item.isEnabled ? 'text-white' : 'text-gray-500'}`}>{item.name}</p>
                        ...
                        <button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${item.isEnabled ? 'bg-orange-500' : 'bg-gray-600'}`}
                        >
            ...
            <div className="flex justify-end gap-4 mt-6 pt-6 border-t border-gray-800 shrink-0">
                <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-white transition-colors cursor-pointer font-medium">Cancel</button>
                <button onClick={handleSave} className="px-6 py-2 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded shadow-lg transition-colors cursor-pointer">Save Layout</button>
            </div>
        </Modal>
    );
}
