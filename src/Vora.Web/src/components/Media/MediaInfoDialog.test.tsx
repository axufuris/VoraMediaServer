...
const remux: MediaPart = {
    ...
    subtitleTracks: [
        ...
        { id: 's2', codec: 'subrip', language: 'eng', isExternal: true, isDownloaded: true, isHearingImpaired: true },
    ],
};
...
describe('media info dialog', () => {
    ...
    // Forced and hearing-impaired are different facts about a track, and a
    // viewer choosing between two English subtitles is choosing on exactly this.
    it('tells forced and hearing-impaired subtitles apart', () => {
        renderDialog([remux]);
        const subtitles = screen.getByRole('heading', { name: 'Subtitles · 2' }).closest('section')!;
        // Each is stated once, and by the track it belongs to: the PGS track is
        // forced, the downloaded SRT is SDH, and neither claims the other's.
        const forced = within(subtitles).getByText('Forced', { selector: 'dt' });
        const sdh = within(subtitles).getByText('Hearing impaired', { selector: 'dt' });

        expect(forced.nextElementSibling).toHaveTextContent('Yes');
        expect(sdh.nextElementSibling).toHaveTextContent('Yes');
        expect(forced.closest('dl')).not.toBe(sdh.closest('dl'));
    });
