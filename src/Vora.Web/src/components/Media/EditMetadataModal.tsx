...
export default function EditMetadataModal({
    ...
    return (
        ...
        >
                ...
                <div className="px-6 pt-6 border-b border-gray-800 bg-gray-950">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-white">Edit {type === 'media' ? 'Metadata' : 'Season'}</h2>
                    ...
                    <div className="flex gap-6 text-sm font-bold text-gray-400">
                        <button onClick={() => setActiveTab('general')} className={`pb-3 border-b-2 transition-colors cursor-pointer ${activeTab === 'general' ? 'border-orange-500 text-orange-400' : 'border-transparent hover:text-white'}`}>
                            General
                        </button>
                        {type === 'media' && (
                            <>
                                <button onClick={() => setActiveTab('poster')} className={`pb-3 border-b-2 transition-colors cursor-pointer ${activeTab === 'poster' ? 'border-orange-500 text-orange-400' : 'border-transparent hover:text-white'}`}>
                                    Posters
                                </button>
                                <button onClick={() => setActiveTab('backdrop')} className={`pb-3 border-b-2 transition-colors cursor-pointer ${activeTab === 'backdrop' ? 'border-orange-500 text-orange-400' : 'border-transparent hover:text-white'}`}>
                                    Backdrops
                                </button>
                            </>
                        )}
                ...
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-gray-900">
                    {activeTab === 'general' && (
                        <form id="metadata-form" onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                ...
                                <input type="text" value={formData.title} onChange={e => handleChange('title', e.target.value)} className="w-full p-2.5 bg-gray-950 rounded border border-gray-700 text-white outline-none focus:border-orange-500 transition-colors" />
                            </div>

                            <div>
                                <div className="flex justify-between items-center mb-1">
                                    <label className="text-sm font-semibold text-gray-300">Sort Title</label>
                                ...
                                <input type="text" value={formData.sortTitle || ''} onChange={e => handleChange('sortTitle', e.target.value)} className="w-full p-2.5 bg-gray-950 rounded border border-gray-700 text-white outline-none focus:border-orange-500 transition-colors" />
                            </div>

                            <div>
                                <div className="flex justify-between items-center mb-1">
                                    <label className="text-sm font-semibold text-gray-300">Overview</label>
                                ...
                                <textarea rows={5} value={formData.overview || ''} onChange={e => handleChange('overview', e.target.value)} className="w-full p-2.5 bg-gray-950 rounded border border-gray-700 text-white outline-none focus:border-orange-500 transition-colors resize-none" />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="date" value={formData.releaseDate || ''} onChange={e => handleChange('releaseDate', e.target.value)} className="w-full p-2.5 bg-gray-950 rounded border border-gray-700 text-white outline-none focus:border-orange-500 transition-colors color-scheme-dark" />
                                </div>

                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="text-sm font-semibold text-gray-300">Content Rating</label>
                                    ...
                                    <input type="text" value={formData.contentRating || ''} onChange={e => handleChange('contentRating', e.target.value)} className="w-full p-2.5 bg-gray-950 rounded border border-gray-700 text-white outline-none focus:border-orange-500 transition-colors" />
                                </div>
                            </div>
                        </form>
                    )}
                    ...
                    {activeTab === 'poster' && (
                        <ArtworkPicker
                            ...
                            actionRowLeft={<p className="text-sm text-gray-400">Select a poster to apply it immediately to the form.</p>}
                            actionRowRight={
                                <button type="button" onClick={() => handleLockToggle('PosterUrl')} className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                    ...
                    {activeTab === 'backdrop' && (
                        <ArtworkPicker
                            ...
                            onDeleteArtwork={deleteArtwork}
                            actionRowLeft={<p className="text-sm text-gray-400">Select a backdrop to apply it immediately to the form.</p>}
                            actionRowRight={
                                <button type="button" onClick={() => handleLockToggle('BackgroundUrl')} className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                ...
                <div className="px-6 py-4 bg-gray-950 border-t border-gray-800 flex justify-end gap-3">
                    <button onClick={onClose} disabled={saving} className="px-5 py-2 text-sm font-bold text-gray-300 hover:text-white transition-colors cursor-pointer">
                        Cancel
                    </button>
                    <button type="submit" form="metadata-form" onClick={activeTab !== 'general' ? handleSubmit : undefined} disabled={saving} className="px-6 py-2 bg-orange-600 hover:bg-orange-500 active:bg-orange-700 disabled:opacity-50 text-white text-sm font-bold rounded shadow transition-colors cursor-pointer">
...
const LockIcon = ({ locked }: { locked: boolean }) => (
    <div className={`p-1.5 rounded transition-colors ${locked ? 'bg-orange-500/20 text-orange-500' : 'bg-gray-800 text-gray-500 hover:text-gray-300'}`}>
        {locked ? (
