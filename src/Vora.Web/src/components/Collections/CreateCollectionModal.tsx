...
export default function CreateCollectionModal({
    ...
    const [activeSection, setActiveSection] = useState<'general' | 'poster' | 'backdrop'>('general');
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="4xl" cardClassName="flex flex-col max-h-[90vh]">
            <ModalHeader title="Create New Collection" onClose={onClose} />

            <ModalBody className="p-6">
                <div className="flex gap-1 border-b border-[var(--vora-border-subtle)] mb-5">
                    {([['general', 'General'], ['poster', 'Posters'], ['backdrop', 'Backdrops']] as const).map(([key, label]) => (
                        <button
                            key={key}
                            type="button"
                            onClick={() => setActiveSection(key)}
                            className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors cursor-pointer ${activeSection === key
                                ? 'border-[var(--vora-accent-500)] text-[var(--vora-accent-500)]'
