...
public class TvdbMetadataProvider : IMetadataProvider, IPluginConnectionTest
{
    ...
    public IEnumerable<PluginSettingDefinitionDto> GetSettingDefinitions()
    {
        return new List<PluginSettingDefinitionDto>
        {
            new PluginSettingDefinitionDto
            {
                ...
                Type = "password",
                Required = true,
                Placeholder = "Paste your TVDB project API key",
                Description = "TVDB v4 API key. Keys are issued per project, not per person — register yours at https://thetvdb.com/api-information and paste it here. Vora exchanges it for a session token automatically and caches that token in plugin settings."
            },
            new PluginSettingDefinitionDto
            {
                Key = TvdbSession.SubscriberPinKey,
                Label = "TVDB Subscriber PIN",
                Type = "password",
                Required = false,
                Placeholder = "Only for a user-supported API key",
                Description = "Leave empty unless TVDB rejects your key without it. A user-supported (free) project key also needs the PIN of a TVDB subscriber, found on your account's subscription page at https://thetvdb.com/subscribe — that PIN is what replaced the old per-user API keys. A licensed key authenticates on its own and needs nothing here."
            }
        };
    }
    ...
    public async Task<PluginConnectionTestResult> TestConnectionAsync(IReadOnlyDictionary<string, string> settings, CancellationToken cancellationToken = default)
    {
        ...
        settings.TryGetValue(TvdbSession.SubscriberPinKey, out var pin);

        var content = new StringContent(TvdbSession.BuildLoginBody(apiKey, pin), Encoding.UTF8, "application/json");
        var response = await _httpClient.PostAsync("login", content, cancellationToken);

        if (response.IsSuccessStatusCode)
        {
            return PluginConnectionTestResult.Ok(string.IsNullOrWhiteSpace(pin)
                ? "TVDB accepted the API key."
                : "TVDB accepted the API key and subscriber PIN.");
        }
        if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
        {
            return PluginConnectionTestResult.Fail(string.IsNullOrWhiteSpace(pin)
                ? "TVDB rejected the API key. If it is a user-supported (free) project key, it also needs a TVDB subscriber PIN — add one below and test again."
                : "TVDB rejected the API key and subscriber PIN. Check both: the key comes from the TVDB API page, the PIN from your subscription page.");
        }
