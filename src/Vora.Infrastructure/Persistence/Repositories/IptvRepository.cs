...
public class IptvRepository : IIptvRepository
{
    ...
    // Answers "would the Live TV / Radio page have anything on it" for both kinds
    // in one pass, because the feature-flags endpoint every client hits on nav
    // load asks for both. The playlist table is small enough to pull the rows
    // that answer the fallback; the channel table is not, so each kind is an
    // EXISTS against the (PlaylistId, Kind) index rather than a count.
    //
    // A channel of the right kind on an active playlist is the strongest signal:
    // it means a playlist was fetched and parsed, and it is what the clients
    // actually render — both the Live TV guide and the Radio tab filter on the
    // per-channel Kind, not the playlist's default. Admin-hidden channels do not
    // count: a playlist whose channels are all hidden shows an empty page.
    // Failing that, a playlist with an M3U URL that has not been scanned yet
    // counts, so adding a source lights the feature up before the first sync.
    public async Task<(bool Tv, bool Radio)> GetSourceAvailabilityAsync(CancellationToken cancellationToken = default)
    {
        var playlists = await _context.IptvPlaylists
            .AsNoTracking()
            .Where(p => p.IsActive)
            .Select(p => new { p.Id, p.DefaultChannelKind, p.M3uUrl })
            .ToListAsync(cancellationToken);

        if (playlists.Count == 0) return (false, false);

        var playlistIds = playlists.Select(p => p.Id).ToList();

        async Task<bool> HasSourceAsync(IptvChannelKind kind) =>
            await _context.IptvChannels
                .AsNoTracking()
                .AnyAsync(c => playlistIds.Contains(c.PlaylistId) && c.Kind == kind && !c.IsHiddenByAdmin, cancellationToken)
            || playlists.Any(p => p.DefaultChannelKind == kind && !string.IsNullOrWhiteSpace(p.M3uUrl));

        return (await HasSourceAsync(IptvChannelKind.Tv), await HasSourceAsync(IptvChannelKind.Radio));
    }
