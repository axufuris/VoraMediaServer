...
public class FFmpegAnalyzerService : IMediaAnalyzerService
{
    ...
    private static (bool IsDefault, bool IsForced, bool IsHearingImpaired) ReadDisposition(JsonElement stream)
    {
        if (!stream.TryGetProperty("disposition", out var disposition)) return (false, false, false);

        return (
            disposition.TryGetProperty("default", out var isDefault) && isDefault.GetInt32() == 1,
            disposition.TryGetProperty("forced", out var isForced) && isForced.GetInt32() == 1,
            disposition.TryGetProperty("hearing_impaired", out var isHearingImpaired) && isHearingImpaired.GetInt32() == 1);
    }

    // Public for the same reason as Hdr10PlusJsonIndicatesDynamicMetadata: it is
    // pure JSON in, track out, and the disposition flags it reads are the part of
    // ffprobe's output most worth pinning down without a media file to hand.
    public static SubtitleTrackInfo ReadSubtitleTrack(JsonElement stream)
    {
        var disposition = ReadDisposition(stream);

        var track = new SubtitleTrackInfo
        {
            StreamIndex = stream.TryGetProperty("index", out var index) ? index.GetInt32() : 0,
            Codec = stream.TryGetProperty("codec_name", out var codec) ? codec.GetString() : null,
            IsDefault = disposition.IsDefault,
            IsForced = disposition.IsForced,
            IsHearingImpaired = disposition.IsHearingImpaired
        };

        if (stream.TryGetProperty("tags", out var tags))
        {
            track.Language = GetTagValue(tags, "language");
            track.Title = GetTagValue(tags, "title");
        }

        if (!track.IsHearingImpaired) track.IsHearingImpaired = SubtitleTraits.TitleSuggestsHearingImpaired(track.Title);
        ...
        try
        {
            ...
            if (root.TryGetProperty("streams", out var streams) && streams.ValueKind == JsonValueKind.Array)
            {
                foreach (var stream in streams.EnumerateArray())
                {
                    ...
                    var (isDefault, isForced, _) = ReadDisposition(stream);
                    ...
                    {
                        result.SubtitleTracks.Add(ReadSubtitleTrack(stream));
                    }
                }
            }
