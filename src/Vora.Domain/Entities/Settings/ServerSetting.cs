...
public class ServerSetting
{
    ...
    // When a title's own audio is in a language other than MetadataLanguage,
    // start it with subtitles in the server's language rather than with none.
    // Off by default: it changes what a viewer sees without them asking for it,
    // and on a library that is all one language it would never fire anyway.
