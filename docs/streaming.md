...
### Which track plays when nobody picked one

`BestPathDecisionManager.PickAutomaticSubtitle` answers this, and only when the client asked for no track — an explicit `requestedSubtitleId` always wins. The baseline is the file's own flags: first `IsForced`, else first `IsDefault`, else none.

`ServerSetting.AutoEnableSubtitlesForForeignAudio` (off by default) adds the case no flag can express — audio in a language this server does not read. When it is on and the part's primary audio language differs from `ServerSetting.MetadataLanguage`, the pick becomes the best track **in the server's language**: forced and not SDH, then forced, then not SDH, then what is left; with nothing in that language, the baseline. Audio with **no language tag** keeps the baseline (most untagged files are ordinary local rips, and reading untagged as foreign would subtitle half a library), and a subtitle with no language is never assumed to be the server's.

Comparison goes through `MetadataLanguageCodes.SameLanguage`, which normalizes 2- vs 3-letter codes, region suffixes (`en-US`), bibliographic vs terminological codes (`ger`/`deu`), and the display names the ffprobe tag reader writes — `eng` is stored as `English`.

**Hearing-impaired is its own flag**, separate from forced: `disposition.hearing_impaired`, the `sdh`/`cc`/`hi` segment on a sidecar, the trait the provider reported on a download, or — since almost nothing sets the disposition — a title that says so (`SubtitleTraits.TitleSuggestsHearingImpaired`).
...
All five settings seed from the environment like any other plugin's — `Vora__PluginSettings__opensubtitles_search__<key>` for `api_key`, `auth_mode`, `username`, `password`, `default_languages` (see the README matrix). `auth_mode` is matched on containing "account", so both the plain word and the dropdown's own label work; anything else stays on the API key.
...
**Downloads land in the server's store, never beside the media**, at `{StoragePaths:Subtitles}/{shard}/{mediaItemId}/{trackId}.vtt`. The library is read-only. The bytes are converted to WebVTT once, on the way in, so the delivery endpoint later serves them as a straight copy — the staged file keeps the source's real extension, because FFmpeg picks its demuxer partly from that.
...
- **Subtitles have their own control.** A CC button beside the gear opens `SubtitleTracksPanel`; **Quality & tracks is video and audio only**. The subtitle panel is **instant-commit** — no Apply — because a text subtitle is cues attached to the playing element and nothing needs confirming. The CC button is hidden entirely when the active part has no subtitle tracks and no provider could fetch one.
