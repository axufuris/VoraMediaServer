...
This file documents the source of truth, the emitter, and the consumption model. Why each ecosystem gets a native client is in [`docs/architecture.md`](../architecture.md).
...
- **Z-index** — modal stacking is web-specific (`docs/design/design-language.md` covers it). Native renderers have their own modal/overlay stacks (Sheet, Popover, fullScreenCover on SwiftUI; ModalBottomSheet, Dialog on Compose).
