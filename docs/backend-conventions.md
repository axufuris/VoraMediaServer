...
- **`ServiceRegistrationExtensions.cs`** — `builder.AddVoraServices()` and its helpers. One helper per concern: `AddVoraDatabase`, `AddVoraRepositories`, `AddVoraManagers`, `AddVoraApplicationServices`, `AddVoraWorkers`, `AddVoraInfrastructure`, `AddVoraEmail`, `AddVoraRealtime`, `AddVoraPluginSystem`, `AddVoraAuthenticationAndAuthorization`, `AddVoraCors`, `AddVoraJsonOptions`. When adding a new manager/service/repository, register it in the helper that matches its kind. `AddVoraEmail` is the cross-cutting bundle for the email subsystem (transport, renderer, queue, secret protector, DataProtection key persistence) — see `docs/email.md`.
...
- **`AuthExtensions.cs`** — `ClaimsPrincipal` extension methods: `GetProfileId`, `GetAccountId`, `HasAllLibraryAccess`, `GetAllowedLibraryIds`, `GetAllowedMovieRatings`, `GetAllowedTvRatings`, `GetAllowedMusicRatings`, `BlockUnratedContent`, `GetMusicAccessFilter`, `GetPlaylistAccessFilter`. Each kind of media is limited only by its own allowlist (empty = open); `BlockUnratedContent` is the one setting they share. **Always** use these instead of re-parsing claims inline.
