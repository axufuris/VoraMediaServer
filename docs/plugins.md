...
TVDB v4 authenticates with a session token obtained by `POST login`, stored in plugin settings as `tvdb_metadata:tvdb_token` and shared by `TvdbMetadataProvider` and `TvdbArtworkProvider`. **Tokens expire after about a month.**

All TVDB auth goes through `TvdbSession` (`Providers/Tvdb/TvdbSession.cs`); don't read `tvdb_token` or build a bearer request directly, and build the login body with `TvdbSession.BuildLoginBody` rather than serializing one inline.

**Two kinds of key, one login endpoint.** TVDB issues keys **per project, not per person** — individual users get a *subscriber PIN* instead, which is what replaced per-user keys. Vora ships no key: each install uses the one its owner registered at `thetvdb.com/api-information`, so nobody shares a rate limit and nothing key-shaped lives in the repo (see `docs/database.md` for why that rule exists).

- A **licensed** key authenticates alone: `{"apikey": "…"}`.
- A **user-supported** (free) key also needs a subscriber's PIN: `{"apikey": "…", "pin": "…"}`, from `tvdb_metadata:subscriber_pin`.

`BuildLoginBody` **omits the `pin` field entirely** when the setting is blank rather than sending `""` — TVDB rejects a login carrying an empty pin, so serializing it unconditionally would break every licensed key the moment the setting existed. That asymmetry is the whole reason the body is built in one shared place.
