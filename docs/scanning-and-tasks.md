...
1. **Scan + enrich, per item** — `TriggerLibraryFolderAndFileScanAsync(libraryId, onItemScanned)` scans, and the scanner invokes the callback as soon as it finishes ingesting an item: **per show** for TV (once all of a show's episodes are in — files are grouped by `GetTvShowFolderName`) and **per movie** for films (deduped so a multi-part movie fires once). The callback enriches that item fully — `TriggerMediaItemMetadataRefreshAsync` (cascades to seasons + episodes) → `TriggerMediaItemArtworkRefreshAsync` → `TriggerMediaItemRatingsRefreshAsync` — in a **fresh DI scope per item** (keeps the EF change-tracker small). So a show's poster fills in mid-scan, right after its episodes are read, instead of after the whole library is scanned.
2. **Enrichment safety net** — `TriggerLibraryEnrichmentAsync(libraryId, forceOverride: false)`. After the interleave nearly everything is already enriched, so this is a cheap no-op walk that only catches stragglers (scan edge cases, items not covered by a callback). It is **always non-force** so a force rescan — where the per-item callback already force-refreshed each item — doesn't double-fetch.
