...
## Full-library workflow order (scan/enrich interleave)

`RunFullLibraryWorkflowAsync` (in `TaskQueueManager`) is the ordered pipeline a library add / full rescan runs. Enrichment is **interleaved with the scan**, not a separate pass after it, so posters appear as content is discovered:

1. **Scan + enrich, per item** — `TriggerLibraryFolderAndFileScanAsync(libraryId, onItemScanned)` scans, and the scanner invokes the callback as soon as it finishes ingesting an item: **per show** for TV (once all of a show's episodes are in — files are grouped by `GetTvShowFolderName`) and **per movie** for films (deduped so a multi-part movie fires once). The callback enriches the item's **show + season** level only — `TriggerMediaItemMetadataRefreshAsync(id, includeEpisodes: false)` (show poster + season posters/metadata via `ProcessTvSeasonsAsync`, season artwork/ratings) → `TriggerMediaItemArtworkRefreshAsync` → `TriggerMediaItemRatingsRefreshAsync` — in a **fresh DI scope per item** (keeps the EF change-tracker small). It **skips per-episode metadata on purpose**: fetching every episode over the network inside the callback would stall the scan (it blocks on N network calls before the next show can scan). So a show's poster and its season posters fill in mid-scan, right after its episodes are read.
2. **Enrichment safety net + episodes** — `TriggerLibraryEnrichmentAsync(libraryId, forceOverride: false)`. This fills in the **episode metadata the callback deferred** (episodes are untouched, so they're in the missing-metadata set) plus any stragglers the callbacks didn't cover. It is **always non-force**: shows/seasons the callback already enriched are skipped, and a force rescan doesn't double-fetch.
3. **Poster overlays** — `RunLibraryOverlaySyncAsync` (a sweep; the badge composites onto the poster the artwork step already set).
4. **Actor metadata** — one global, whole-DB pass. This **must** stay a single deferred pass, not per-item: it's not library-scoped, so running it per item would redo every actor N times (see also the per-file watcher path, which drops it entirely).
5. **Analyze media + detect markers** — the heavy FFmpeg passes (`TriggerLibraryFileAnalysisAsync`, then `TriggerLibrarySilenceDetectionAsync`). They don't affect posters and season marker clustering needs the whole season present, so they run **last**, after the library is visually populated. **Video preview thumbnails** aren't in this workflow at all — they run on their own schedule (`VideoThumbnailScheduleTime`, see `docs/video-thumbnails.md`).

The scanner exposes the hook as an optional `Func<Guid, Task>?` on `ScanTvShowLibraryAsync` / `ScanMovieLibraryAsync` (and threaded through `TriggerLibraryFolderAndFileScanAsync`) — a dependency-free callback, so the scanner plugin only reports "item X is done" and never learns about metadata.
