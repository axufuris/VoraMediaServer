...
**The artwork sweep is batched, not per-item.** `RemoveThumbnailsForSource` probes 4 kind folders x 6 width buckets x 2 extensions = **48 `File.Exists` calls per source**, nearly all misses; on a bind-mounted volume each is a round trip. Called once per poster and background across a whole library that dominated the delete. `RemoveThumbnailsForSources` lists each kind folder **once** and tests membership, and `SweepPhysicalOverlays` does the same for the overlay directory. Keep bulk paths on the bulk methods.

**Deleting a library purges thumbnail FILES only.** `DeleteLibraryAsync` calls `PurgeLibraryThumbnailFilesAsync`, not `PurgeLibraryThumbnailsAsync`. The latter reads each item back (with its parts) and calls `SaveChanges` **per item** on one scoped `DbContext`, so every save re-runs change detection over everything tracked so far — quadratic. On a library whose rows are about to be removed by the repository's bulk `ExecuteDeleteAsync` moments later, nulling their thumbnail columns is work thrown away; on a music library it was thousands of round trips clearing video-thumbnail columns on tracks that never had one. Don't point the delete path back at the column-resetting purge.
