...
2. **Scan + enrich each unit, in parallel** — `Parallel.ForEachAsync` over the units at `Math.Clamp(ProcessorCount, 2, 6)` degree. Each unit runs `ILibraryManager.ScanAndEnrichUnitAsync`, which:
   ...
   - **Episodes enrich in parallel within the show.** For a TV show, `TriggerMediaItemMetadataRefreshAsync` fetches the show + seasons on the shared scope, then runs the show's episodes through `Parallel.ForEachAsync` at `EpisodeEnrichmentConcurrency` (4) — **each episode in its own fresh scope** (own `DbContext`, thread-safe). This matters on a first scan: a show can have hundreds of episodes, and one-at-a-time network fetches (each bounded only by the default 100s HTTP timeout on a slow/unmatched lookup) let a single episode-heavy show gate the whole library scan for hours. The effective ceiling is `units × 4`; shared-row writes stay serialized by `ReferenceWriteGate`, and a failing episode is caught + logged so it can't abort the rest of the show.
