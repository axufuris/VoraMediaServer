...
- **Long loops must observe the token, and the methods they call must accept one.** The gap is rarely a loop ignoring a token it was given — it is a method that takes no token at all, so nothing can interrupt it. `PurgeLibraryThumbnailsAsync` was the example: a per-item loop with no parameter to check. The one deliberate exception is `ScheduledJobWorker.CheckAndRunSchedulesAsync`, whose caller wraps it in `catch (Exception)` and logs an error — throwing on cancel there would log a spurious error on every shutdown, and its loop is a bounded check-and-queue.
...
**The music write phase is the slow half, and it reports progress.** `IngestMusicFilesAsync` parses tags per file, then writes artists, albums, tracks and parts. The write loop used to report nothing, so a large library sat on `Scanning … (n/n)` for all of it and looked hung; it reports `Saving <artist> - <album> (n/total)` now. Each track costs several round trips, so a few thousand tracks is tens of thousands of queries, and two things stop that compounding: **`ReleaseTrackedEntitiesAsync` after each album** (every `SaveChanges` re-runs change detection over everything tracked so far, so one `DbContext` holding a library's entities slows with each album — the loop keeps only ids across albums, so clearing is safe), and **`syncEdition: false` on a track's part** (`Edition` is Director's Cut / IMAX; the sync loads the item *with its parts* to pick the best, and a track has none). The preserved-user-data probe is deliberately **not** memoized: a trash purge can write that table mid-scan, and caching "empty" drops the restore (`MediaRepositoryPreservationTests` covers it). Albums group by **tag, not folder**.
...
Not a style preference: the two held private copies and the watcher's had no audio formats, so a monitored music library reported itself watched and discarded every event. `FolderWatcherReconciliationTests` asserts with `BeSameAs` (reference identity) that the watcher hands back the shared lists, so reintroducing a literal fails the build.
...
**Reconciliation does not fan out over a backlog.** The sweep queues one task per un-ingested file — right for a few stragglers, wrong for a backlog, since a freshly added library has *every* file un-ingested and the fan-out merely duplicates the full scan the add already queued (and each music task re-reads the whole ingested-path set, making it quadratic). `ShouldQueueFullScan` sends a library with nothing ingested, or a backlog past `ReconcileFanOutLimit`, to a **single** `QueueScanLibrary`.

**Deleting a library purges thumbnail FILES only, and the artwork sweep is batched.** `DeleteLibraryAsync` calls `PurgeLibraryThumbnailFilesAsync`, not `PurgeLibraryThumbnailsAsync` — the latter reads each item back and calls `SaveChanges` **per item** on one `DbContext` (quadratic, as above) to null columns on rows the bulk `ExecuteDeleteAsync` removes moments later. Likewise `RemoveThumbnailsForSource` probes 4 kind folders x 6 widths x 2 extensions = **48 `File.Exists` per source**, nearly all misses and each a round trip on a bind mount; `RemoveThumbnailsForSources` and `SweepPhysicalOverlays` list their directory **once** and test membership. Keep the delete path on the file-only purge, and bulk callers on the bulk methods.

**Deleting a library cancels its queued work first.** Every ingestion task shares the library's `library:{id}` key and the dispatcher is FIFO within a key, so `QueueDeleteLibrary` used to sit *behind* everything queued — scans of the very thing being deleted ran to completion first. It calls `CancelTasksForLibrary` **before** enqueuing (after, and it cancels itself), and `DeleteLibraryAsync` calls `StopWatching` so the watcher stops polling a library that no longer exists.
