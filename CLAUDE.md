...
- [`docs/scanning-and-tasks.md`](docs/scanning-and-tasks.md) — background task queue (`ITaskQueueManager` + `TaskProcessingWorker`), per-task cancellation, lazy task-name resolution, folder watcher → per-file ingest (`QueueScanNewFile`), single-file scanner methods + new-season metadata refresh, live `EpisodeCount`
...
- **Storage paths added since the original deployment**: `StoragePaths:Logs` (file sink under `<base>/logs`, see `docs/logs.md`) and `StoragePaths:Backups` (zip backups under `<base>/backups`, see `docs/backups.md`). The default `docker-compose.yml` puts both under the existing `./Vora-data:/app/data` mount so they survive container rebuilds.
