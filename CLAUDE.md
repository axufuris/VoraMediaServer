...
- [`docs/scanning-and-tasks.md`](docs/scanning-and-tasks.md) — background task queue (`ITaskQueueManager` + `TaskProcessingWorker`), per-task cancellation, per-item progress (`ITaskProgressReporter`), lazy task-name resolution, folder watcher → per-file ingest (`QueueScanNewFile`) + `ExcludeFilters`, single-file scanner methods + new-season metadata refresh, duplicate-item prevention, per-part `Edition` + version picker, soft-delete / Media Trash + auto-purge, live `EpisodeCount`
