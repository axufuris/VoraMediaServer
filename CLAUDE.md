...
- **Text subtitles ship as a sidecar `.vtt`, not in the HLS**: the transcode emits `-sn` unconditionally. Image subs burn into the video; text subs are extracted to a **content-keyed cache** at `{mediaPartId}_{subtitleTrackId}.vtt` in the same directory. **Never run that extraction in the request path** — `/start` returns a cache hit's URL or starts background work and returns null; a large remux takes minutes and blocking timed the client out. `MediaSubtitleTrack.StreamIndex` is the **absolute** stream index (`-map 0:N`), not the subtitle-relative one (`0:s:N`) — don't mix the two. See `docs/streaming.md`.
