...
- **Session output files are namespaced by filename, not by directory**: everything a stream session produces lands flat in `TranscoderTempDirectory` prefixed with the transcode key (`session.ExtraId ?? session.MediaItemId`) — `{key}.m3u8`, `{key}_{n}.ts`, `{key}_subtitles.vtt`. `GET /api/streaming/hls/s/{token}/{fileName}` serves a file only when the name starts with the prefix its token signs, so a new artifact **must** be named `{key}…` to be reachable, and must **not** end in `_<integer>` or `TryParseSegmentRequest` treats it as an HLS segment. See `docs/streaming.md`.
- **Text subtitles ship as a sidecar `.vtt`, not in the HLS**: the transcode emits `-sn` unconditionally. Image subs burn into the video; text subs are extracted once at session start and surfaced as `StartStreamResponse.SubtitleUrl`. `MediaSubtitleTrack.StreamIndex` is the **absolute** stream index (`-map 0:N`), not the subtitle-relative one (`0:s:N`) — don't mix the two. See `docs/streaming.md`.
