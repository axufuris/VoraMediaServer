...
- [`docs/streaming.md`](docs/streaming.md) — playback path (decision → session → HLS), the flat prefix-namespaced transcode directory + signed-prefix file route, subtitle strategies (`BurnIn` for image subs, sidecar WebVTT for text), the sidecar extraction pipeline (`ISubtitleExtractionService`, absolute vs subtitle-relative `-map`, cleanup semantics)
...
- [`docs/collections.md`](docs/collections.md) — collection content sync + chronological ordering: the two-provider model (`ContentSyncExternalId` vs `ExternalListId`), AI List (`openai_list`) franchise/universe scope + completeness passes, `CollectionMembershipResolver` title matching + **all-library-seasons expansion**, mirror mode + `ExcludedMediaIdsJson` + `ManuallyAdded`, empty-sync admin alerts, chronology (`openai_chronology`) setYear passes (seed → batch → verify → anchor → repair → distinct) + `InUniverseYear`/`InUniverseYearLocked` per-item year lock, what to type in each describe field, per-collection task serialization by `CollectionKey`, the `collections_model` setting (gpt-5 default; reasoning models skip `temperature`) + the LLM freshness limit and the paths to more accuracy (curated Trakt/MDbList/IMDb providers now, a web-search-grounded Perplexity provider as a future option)
...
- [`docs/redesign/design-language.md`](docs/redesign/design-language.md) — cinematic design language: client primitives (`MediaCard`, `MediaRow`, `MediaGrid`, `PersonCard`, `CastRow`, `VideoCard`, `CinematicBackdrop`, `LetterRail`, `QualityPanel`, etc.), the media-card metric tokens, modal stacking, brand assets
...
- [`docs/clients/primitive-specs.md`](docs/clients/primitive-specs.md) — cross-platform contract for cinematic primitives (`MediaCard`, `MediaRow`, `PersonCard`, `VideoCard`, `CinematicBackdrop`, `Glass`, `QualityPanel`, `NowPlayingBar`, …); prop shapes, observable behavior, TV focus rules
...
- **One media tile, one row, one detail hero.** Every poster/tile in the client is `components/Client/Primitives/MediaCard.tsx`; every horizontal row is `MediaRow.tsx`; every wrapping grid is `MediaGrid.tsx`; people are `PersonCard`/`CastRow`; trailers and extras are `VideoCard`; the top of a detail page is `DetailHero.tsx` (+ `HeroChip`), shared by the media details and discovery details pages. Don't hand-roll a card, a scroller, or a detail header. What shows under a tile comes from `utils/posterCaption.ts` keyed on the item's type — add a case there rather than passing bespoke subtitles.
- **Card and row sizes come from tokens, not literals.** `--vora-card-w-sm|md|lg`, `--vora-card-min-w`, `--vora-card-gap`, `--vora-card-title-size`, `--vora-card-caption-size`, `--vora-person-w`, `--vora-video-w`, `--vora-row-gutter` in `styles/tokens.css`. They're `rem`/`clamp()` so the whole client scales with the root font size and the viewport. Never size a tile in `px`.
...
- **localStorage keys**: see `docs/auth-and-devices.md` — the canonical admin flag is `is_server_admin`, NOT `is_admin` (that key is dead).
...
- **Session output files are namespaced by filename, not by directory**: everything a stream session produces lands flat in `TranscoderTempDirectory` prefixed with the transcode key (`session.ExtraId ?? session.MediaItemId`) — `{key}.m3u8`, `{key}_{n}.ts`. `GET /api/streaming/hls/s/{token}/{fileName}` serves a file only when the name starts with the prefix its token signs, so a new artifact **must** be named with the prefix its URL's token signs, and must **not** end in `_<integer>` or `TryParseSegmentRequest` treats it as an HLS segment. See `docs/streaming.md`.
- **Text subtitles come from their own route, never from `/start`**: the transcode emits `-sn` unconditionally, image subs burn into the video, and text subs are served by `GET /api/streaming/sessions/{sessionId}/subtitle/{trackId}.vtt` from a content-keyed cache at `{TranscoderTempDirectory}/subcache/{partId}_{trackId}_{fingerprint}.vtt`, warmed after a scan by `ISubtitlePreExtractionManager` (`ServerSetting.PreExtractSubtitlesOnScan`, default on, independent of thumbnails) and extracted on demand otherwise. The fingerprint (source size + mtime + stream index) lives in the **file name**, so a changed source is a miss by construction — never add an age- or size-based eviction, the cache is keep-forever by design. **Never put extraction back on the play path**: a large remux takes minutes and blocking `/start` timed the client out. `StartStreamResponse.SubtitleUrl` is kept on the DTO but always null. `MediaSubtitleTrack.StreamIndex` is the **absolute** stream index (`-map 0:N`), not the subtitle-relative one (`0:s:N`) — don't mix the two, and the `0:s:N` ordinal counts **container streams only**, so sidecar rows must be excluded before indexing. A track is embedded or **external** (`ExternalFilePath` set, a `.srt`/`.ass`/`.vtt` beside the video): the two reconcile separately — `SyncMediaTracksAsync` by stream index for embedded, `SyncExternalSubtitleTracksAsync` by path for sidecars — because an ffprobe pass reconciling both would delete every sidecar. See `docs/streaming.md`.
