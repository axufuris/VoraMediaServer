...
| `tvdb_metadata` | The TV Database (TVDB) | `api_key`, `subscriber_pin` |
...
**A note on the TVDB key.** TVDB v4 issues API keys **per project, not per
person** — register your own at
[thetvdb.com/api-information](https://thetvdb.com/api-information). Vora ships
no key of its own, so each install uses the one its owner registered, and
nobody shares a rate limit. There are two kinds:

- A **licensed** key authenticates on its own. Set `api_key` and leave
  `subscriber_pin` empty.
- A **user-supported** (free) key also needs the PIN of a TVDB subscriber,
  shown on your account's subscription page. Set both.

If TVDB rejects a key that looks correct, a missing PIN is the usual reason —
Test Connection on the plugin page says so explicitly.
...
```yaml
    environment:
      ...
      # Only for a user-supported (free) TVDB key; a licensed key needs no PIN.
      # Vora__PluginSettings__tvdb_metadata__subscriber_pin: "YOUR_TVDB_SUBSCRIBER_PIN"
...
- [`docs/collections.md`](docs/collections.md) — collection content
  ...
  ordering
- [`docs/design/`](docs/design/) — client templates, scheduling, and
