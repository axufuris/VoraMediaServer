...
public class SubtitleSearchManagerTests
{
    ...
    public async Task No_installed_provider_means_the_feature_is_unavailable()
    {
        ...
        (await NewManager().IsAvailableAsync(TestContext.Current.CancellationToken)).Should().BeFalse();
    }
    ...
    public async Task An_unconfigured_provider_means_the_feature_is_unavailable()
    {
        ...
        (await NewManager(Provider("opensubtitles_search", configured: false)).IsAvailableAsync(TestContext.Current.CancellationToken)).Should().BeFalse();
    }
    ...
    public async Task A_configured_provider_makes_the_feature_available()
    {
        ...
        (await NewManager(Provider("opensubtitles_search", configured: true)).IsAvailableAsync(TestContext.Current.CancellationToken)).Should().BeTrue();
    }
    ...
    public async Task The_admin_selected_provider_is_used()
    {
        ...
        await NewManager(first, second).SearchAsync(MediaId, ["en"], TestContext.Current.CancellationToken);
    ...
    public async Task An_unusable_selection_falls_back_to_a_provider_that_works()
    {
        ...
        await NewManager(unconfigured, working).SearchAsync(MediaId, ["en"], TestContext.Current.CancellationToken);
    ...
    public async Task A_provider_that_throws_while_reporting_readiness_is_skipped()
    {
        ...
        (await NewManager(broken, working).IsAvailableAsync(TestContext.Current.CancellationToken)).Should().BeTrue();
    }
    ...
    public async Task Searching_with_no_provider_returns_nothing_rather_than_throwing()
    {
        ...
        (await NewManager().SearchAsync(MediaId, ["en"], TestContext.Current.CancellationToken)).Should().BeEmpty();
    }
    ...
    public async Task An_episode_is_searched_by_its_series_and_numbering()
    {
        ...
        await NewManager(provider).SearchAsync(MediaId, ["en"], TestContext.Current.CancellationToken);
    ...
    public async Task A_movie_is_searched_by_its_own_title_with_no_numbering()
    {
        ...
        await NewManager(provider).SearchAsync(MediaId, ["en", "es"], TestContext.Current.CancellationToken);
    ...
    public async Task A_download_becomes_a_selectable_track_in_the_server_store()
    {
        ...
        var result = await NewManager(provider).DownloadAndAttachAsync(MediaId, "9001", "en", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task A_downloaded_file_is_written_under_the_server_subtitle_store()
    {
        ...
        await NewManager(provider).DownloadAndAttachAsync(MediaId, "9001", "en", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task A_provider_that_returns_nothing_attaches_no_track()
    {
        ...
        (await NewManager(provider).DownloadAndAttachAsync(MediaId, "9001", "en", cancellationToken: TestContext.Current.CancellationToken)).Should().BeNull();
        await _media.DidNotReceive().AddSubtitleTrackAsync(Arg.Any<MediaSubtitleTrack>());
    }
