...
public class OpenSubtitlesAuthTests : IDisposable
{
    ...
    public async Task Api_key_mode_never_signs_in()
    {
        ...
        await NewProvider(ApiKeyOnly()).SearchAsync(Query(), TestContext.Current.CancellationToken);
    ...
    public async Task Blank_credentials_in_account_mode_still_search_anonymously()
    {
        ...
        await NewProvider(Account(username: null)).SearchAsync(Query(), TestContext.Current.CancellationToken);
    ...
    public async Task Account_mode_signs_in_then_carries_the_bearer()
    {
        ...
        await NewProvider(Account()).SearchAsync(Query(), TestContext.Current.CancellationToken);
    ...
    public async Task A_download_after_re_login_still_carries_its_body()
    {
        ...
        await NewProvider(Account()).DownloadAsync("9001", TestContext.Current.CancellationToken);
    ...
    public async Task A_rejected_sign_in_leaves_the_search_empty_rather_than_throwing()
    {
        ...
        var results = await NewProvider(Account()).SearchAsync(Query(), TestContext.Current.CancellationToken);
