...
public class SettingsRoundTripTests : IClassFixture<VoraApiTestFactory>
{
    ...
    public async Task Feature_flags_PUT_then_GET_round_trips_each_flag()
    {
        ...
        // Live TV and Internet Radio are reported as "switched on AND usable".
        // What round-trips is the stored toggle; this server has no IPTV source,
        // so the derived pair reads false however they were saved.
        flags.LiveTvEnabled.Should().BeFalse();
        flags.InternetRadioEnabled.Should().BeTrue();
        flags.LiveTv.Should().BeFalse();
        flags.InternetRadio.Should().BeFalse();
    }
