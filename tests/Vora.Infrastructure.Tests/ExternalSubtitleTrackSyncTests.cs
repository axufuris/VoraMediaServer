...
public class ExternalSubtitleTrackSyncTests
{
    ...
    private static MediaSubtitleTrack External(string path, string? language = null, bool forced = false, bool hearingImpaired = false) =>
        new() { ExternalFilePath = path, Codec = "subrip", Language = language, IsForced = forced, IsHearingImpaired = hearingImpaired, MediaPartId = PartId };
    ...
    // Renaming "Movie.en.srt" to "Movie.en.sdh.srt" is the whole of how a sidecar
    // declares itself hearing-impaired, and the row is matched by path, so the
    // rename has to carry the flag onto the existing track.
    [Fact]
    public async Task A_sidecar_renamed_to_sdh_is_marked_hearing_impaired()
    {
        using var db = NewContext();
        Seed(db, External("/media/Movie.en.srt", language: "en"));

        await Repo(db).SyncExternalSubtitleTracksAsync(PartId, [External("/media/Movie.en.srt", language: "en", hearingImpaired: true)]);

        Tracks(db).Should().ContainSingle().Which.IsHearingImpaired.Should().BeTrue();
    }

    [Fact]
    public async Task An_embedded_track_that_gains_the_sdh_flag_is_updated_in_place()
    {
        using var db = NewContext();
        Seed(db, Embedded(2));

        var reprobed = Embedded(2);
        reprobed.IsHearingImpaired = true;
        await Repo(db).SyncMediaTracksAsync(PartId, [], [], [reprobed]);
