...
public class FFmpegSubtitleExtractionTests : IDisposable
{
    ...
    public async Task Extraction_gates_do_not_accumulate_across_requests()
    {
        ...
        for (var i = 0; i < 25; i++)
        {
            await service.GetOrExtractWebVttAsync(SubtitleSource.Embedded(source, 3, 1), _root, Guid.NewGuid(), Guid.NewGuid(), TestContext.Current.CancellationToken);
        }
    ...
    public async Task A_gate_is_released_even_when_extraction_fails()
    {
        ...
        await service.GetOrExtractWebVttAsync(SubtitleSource.Embedded("/no/such/source.mkv", 3, 1), _root, PartId, TrackId, TestContext.Current.CancellationToken);
    ...
    public async Task A_valid_cache_entry_is_returned_without_extracting()
    {
        ...
        var result = await NewService().GetOrExtractWebVttAsync(SubtitleSource.Embedded(source, 3, 1), _root, PartId, TrackId, TestContext.Current.CancellationToken);
