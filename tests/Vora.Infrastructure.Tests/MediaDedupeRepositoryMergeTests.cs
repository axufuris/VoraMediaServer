...
public class MediaDedupeRepositoryMergeTests
{
    ...
    public async Task Merges_two_show_rows_moving_parts_and_watch_state()
    {
        ...
        Assert.Null(await db.Set<TvShow>().FirstOrDefaultAsync(t => t.Id == dropId, TestContext.Current.CancellationToken));
        Assert.NotNull(await db.Set<TvShow>().FirstOrDefaultAsync(t => t.Id == keepId, TestContext.Current.CancellationToken));

        // each keeper episode now carries both the 1080p and 4K parts
        Assert.Equal(2, await db.Set<MediaPart>().CountAsync(p => p.MediaItemId == k1.Id, TestContext.Current.CancellationToken));
        Assert.Equal(2, await db.Set<MediaPart>().CountAsync(p => p.MediaItemId == k2.Id, TestContext.Current.CancellationToken));
        Assert.Equal(0, await db.Set<MediaPart>().CountAsync(p => p.MediaItemId == d1.Id || p.MediaItemId == d2.Id, TestContext.Current.CancellationToken));

        // E1 conflict merged: played wins, max resume kept
        var e1 = await db.Set<UserMediaState>().SingleAsync(s => s.MediaItemId == k1.Id && s.ProfileId == profileId, TestContext.Current.CancellationToken);
        ...
        var e2 = await db.Set<UserMediaState>().SingleAsync(s => s.MediaItemId == k2.Id && s.ProfileId == profileId, TestContext.Current.CancellationToken);
        Assert.True(e2.IsPlayed);
        Assert.Equal(300, e2.ResumePositionSeconds);
        Assert.False(await db.Set<UserMediaState>().AnyAsync(s => s.MediaItemId == d2.Id, TestContext.Current.CancellationToken));
    }
    ...
    public async Task Merge_re_points_collection_membership_from_the_dropped_episode_to_the_keeper()
    {
        ...
        Assert.False(await db.Set<CollectionItem>().AnyAsync(ci => ci.MediaItemId == d1Id, TestContext.Current.CancellationToken));
        var moved = await db.Set<CollectionItem>().SingleAsync(ci => ci.CollectionId == collectionId && ci.MediaItemId == k1Id, TestContext.Current.CancellationToken);
    ...
    public async Task Merge_drops_the_duplicate_membership_when_the_keeper_is_already_in_the_collection()
    {
        ...
        Assert.False(await db.Set<CollectionItem>().AnyAsync(ci => ci.MediaItemId == d1Id, TestContext.Current.CancellationToken));
        Assert.Equal(1, await db.Set<CollectionItem>().CountAsync(ci => ci.CollectionId == collectionId, TestContext.Current.CancellationToken));
        Assert.True(await db.Set<CollectionItem>().AnyAsync(ci => ci.CollectionId == collectionId && ci.MediaItemId == k1Id, TestContext.Current.CancellationToken));
    }
    ...
    public async Task Moves_drop_only_episode_onto_the_keeper()
    {
        ...
        var moved = await db.Set<Episode>().Include(e => e.Season).FirstAsync(e => e.Id == d3.Id, TestContext.Current.CancellationToken);
        Assert.Equal(keepId, moved.Season.TvShowId);
        Assert.Equal(1, await db.Set<MediaPart>().CountAsync(p => p.MediaItemId == d3.Id, TestContext.Current.CancellationToken));
    }
    ...
    public async Task Does_not_merge_shows_in_different_libraries()
    {
        ...
        Assert.Equal(2, await db.Set<TvShow>().CountAsync(TestContext.Current.CancellationToken));
    }
}
