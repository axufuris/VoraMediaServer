...
public class MediaRepositoryPreservationTests
{
    ...
    public async Task DeleteThenReAdd_RestoresRatingAndWatchState_ByExternalId()
    {
        ...
        Assert.Equal(1, await db.PreservedUserMediaData.CountAsync(TestContext.Current.CancellationToken));

        var readded = new Movie { Title = "The Matrix", LibraryId = libraryId, TmdbId = "603" };
        await repo.AddMediaItemAsync(readded);

        var rating = await db.UserMediaRatings.FirstOrDefaultAsync(r => r.MediaItemId == readded.Id && r.ProfileId == profileId, TestContext.Current.CancellationToken);
        var state = await db.UserMediaStates.FirstOrDefaultAsync(s => s.MediaItemId == readded.Id && s.ProfileId == profileId, TestContext.Current.CancellationToken);

        Assert.NotNull(rating);
        Assert.Equal(9m, rating!.Rating);
        Assert.NotNull(state);
        Assert.Equal(1234, state!.ResumePositionSeconds);
        Assert.Equal(0, await db.PreservedUserMediaData.CountAsync(TestContext.Current.CancellationToken));
    }
    ...
    public async Task ReAdd_WithDifferentExternalId_DoesNotRestore()
    {
        ...
        Assert.False(await db.UserMediaRatings.AnyAsync(r => r.MediaItemId == unrelated.Id, TestContext.Current.CancellationToken));
        Assert.Equal(1, await db.PreservedUserMediaData.CountAsync(TestContext.Current.CancellationToken));
    }
    ...
    public async Task SyncItemEditionFromParts_UsesBestResolutionPartsEdition()
    {
        ...
        var refreshed = await db.MediaItems.FindAsync([movie.Id], TestContext.Current.CancellationToken);
